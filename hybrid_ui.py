...
# Gemini 키는 secrets의 단일 키를 공통 사용
...
def render_hybrid_system():
    st.subheader("🤖 생산 계획 하이브리드 수사 시스템")
...
""")
    ...
    if prompt := st.chat_input("질문을 입력하세요 (예: 1/6 조립1 70%만 생산하고 싶어)", key="hybrid_input"):
        ...
        if not target_date:
            ...
            st.session_state.hybrid_messages.append({"role": "assistant", "content": answer})
